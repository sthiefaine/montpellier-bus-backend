...
/**
 * Vérifie si l'heure actuelle correspond à 23h58 heure française
 */
  ...
  try {
    ...
    if (!isCorrectFrenchTime()) {
      return res.status(400).json({
        message: "Le job ne peut être exécuté qu'à 23h58 heure française",
      });
    }
...
const deletePreviousNightData = async (req, res) => {
  try {
    // Vérifier si l'heure est correcte en France
    if (!isCorrectFrenchTime()) {
      console.log("Le job n'est pas exécuté car l'heure n'est pas 23h58 heure française");
      return res.status(400).json({
        message: "Le job ne peut être exécuté qu'à 23h58 heure française",
      });
    }
